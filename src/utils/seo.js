...
export const extractKeywords = (
 ...
 const words = allText
  ...
  .filter(
   (word) =>
    word.length > 3 && !commonWords.includes(word) && !word.match(/^\d+$/) // Exclude pure numbers
  )
...
export const validateSEO = (data) => {
 ...
 if (!data.description) {
  ...
  recommendations.push(
   "Meta description could be longer (120-160 characters recommended)"
  )
 ...
 if (data.content) {
  ...
  if (readingTime < 3) {
   recommendations.push(
    "Content might be too short for good SEO (aim for 300+ words)"
   )
  }
 }
